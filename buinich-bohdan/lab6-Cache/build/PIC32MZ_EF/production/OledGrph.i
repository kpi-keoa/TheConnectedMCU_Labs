...
OledMoveTo(int xco, int yco)
 ...
 pbOledCur = &rgbOledBmp[((yco/8) * 128) + xco];
 bnOledCur = yco & 7;
...
OledFillRect(int xco, int yco)
 ...
 while (ycoTop <= ycoBottom) {
  ...
  pbLeft = &rgbOledBmp[((ycoTop/8) * 128) + xcoLeft];
...
OledGetBmp(int dxco, int dyco, BYTE * pbBits)
 ...
 bnAlign = ycoTop & 0x07;
 pbDspLeft = &rgbOledBmp[((ycoTop/8) * 128) + xcoLeft];
 pbBmpLeft = pbBits;
...
OledMoveLeft()
 ...
 if (((pbOledCur - rgbOledBmp) & (128 -1)) == 0) {
  return;
 }
...
OledMoveRight()
 ...
 if (((pbOledCur-rgbOledBmp) & (128 -1)) == (128 -1)) {
  return;
 }
