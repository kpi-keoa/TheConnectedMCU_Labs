...
FSTATUS pin_init()
{
	...
	//  set input mode for BTN_1_SHIELD
	TRISAbits.TRISA3 = 1;  // set direction to input

	//  set input mode for BTN_2_SHIELD
	TRISDbits.TRISD5 = 1;  // set direction to input

	//  set input mode for BTN_3_SHIELD
	TRISFbits.TRISF1 = 1;  // set direction to input

	//  set input mode for BTN_4_SHIELD
	TRISAbits.TRISA2 = 1;  // set direction to input
...
FSTATUS pin_write(enum pin_e pin, enum pin_state_e state)
{
	switch (pin)
	{
		case LED_1_PIN: LATGbits.LATG6  = (state == HIGH_STATE ? 1 : 0); break;
		case LED_2_PIN: LATDbits.LATD4  = (state == HIGH_STATE ? 1 : 0); break;
		case LED_3_PIN: LATBbits.LATB11 = (state == HIGH_STATE ? 1 : 0); break;
		case LED_4_PIN: LATGbits.LATG15 = (state == HIGH_STATE ? 1 : 0); break;
...
{
	FCHECK(pin_write(LED_1_PIN, (leds & LED_1_ENABLE ? HIGH_STATE : LOW_STATE)));
	FCHECK(pin_write(LED_2_PIN, (leds & LED_2_ENABLE ? HIGH_STATE : LOW_STATE)));
	FCHECK(pin_write(LED_3_PIN, (leds & LED_3_ENABLE ? HIGH_STATE : LOW_STATE)));
	FCHECK(pin_write(LED_4_PIN, (leds & LED_4_ENABLE ? HIGH_STATE : LOW_STATE)));
}
...
enum pin_state_e pin_read(enum pin_e pin)
{
	switch (pin)
	{
		...
		case BTN_1_SHIELD_PIN: return (PORTAbits.RA3   ? HIGH_STATE : LOW_STATE);
		case BTN_2_SHIELD_PIN: return (PORTDbits.RD5   ? HIGH_STATE : LOW_STATE);
		case BTN_3_SHIELD_PIN: return (PORTFbits.RF1   ? HIGH_STATE : LOW_STATE);
		case BTN_4_SHIELD_PIN: return (PORTAbits.RA2   ? HIGH_STATE : LOW_STATE);
	}
	return LOW_STATE;
}
